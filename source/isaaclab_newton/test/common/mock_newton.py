...
class MockNewtonArticulationView:
    ...
    def __init__(
        ...
        if is_fixed_base:
            ...
            self._link_accelerations = wp.zeros(
                (num_instances, 1, num_bodies), dtype=wp.spatial_vectorf, device=device
            )
        ...
        self._dof_accelerations = wp.zeros((num_instances, 1, num_joints), dtype=wp.float32, device=device)
    ...
    def get_link_accelerations(self, state) -> wp.array | None:
    ...
    def set_root_transforms(self, state, transforms: wp.array):
        if transforms.shape != self._root_transforms.shape:
            transforms = transforms.reshape(self._root_transforms.shape)
    ...
    def set_root_velocities(self, state, velocities: wp.array):
        if self._root_velocities is None:
            return
        if velocities.shape != self._root_velocities.shape:
            velocities = velocities.reshape(self._root_velocities.shape)
    ...
    def set_mock_data(
        ...
        def _reshape_to(arr: wp.array, shape: tuple[int, ...]) -> wp.array:
            return arr if arr.shape == shape else arr.reshape(shape)

        # --- root
        if root_transforms is None:
            self._root_transforms.zero_()
        else:
            self._root_transforms.assign(_reshape_to(root_transforms, self._root_transforms.shape))

        if root_velocities is None:
            if self._root_velocities is not None:
                self._root_velocities.zero_()
        else:
            if self._root_velocities is None:
                shape = (self._count, 1, 1) if self._is_fixed_base else (self._count, 1)
                self._root_velocities = wp.zeros(shape, dtype=wp.spatial_vectorf, device=self._device)
            self._root_velocities.assign(_reshape_to(root_velocities, self._root_velocities.shape))

        # --- links
        if link_transforms is None:
            self._link_transforms.zero_()
        else:
            self._link_transforms.assign(_reshape_to(link_transforms, self._link_transforms.shape))

        if link_velocities is None:
            if self._link_velocities is not None:
                self._link_velocities.zero_()
        else:
            if self._link_velocities is None:
                self._link_velocities = wp.zeros(
                    (self._count, 1, self._link_count), dtype=wp.spatial_vectorf, device=self._device
                )
            self._link_velocities.assign(_reshape_to(link_velocities, self._link_velocities.shape))

        if link_accelerations is None:
            if self._link_accelerations is not None:
                self._link_accelerations.zero_()
        else:
            if self._link_accelerations is None:
                self._link_accelerations = wp.zeros(
                    (self._count, 1, self._link_count), dtype=wp.spatial_vectorf, device=self._device
                )
            self._link_accelerations.assign(_reshape_to(link_accelerations, self._link_accelerations.shape))

        # --- joints
        if dof_positions is None:
            self._dof_positions.zero_()
        else:
            self._dof_positions.assign(_reshape_to(dof_positions, self._dof_positions.shape))

        if dof_velocities is None:
            self._dof_velocities.zero_()
        else:
            self._dof_velocities.assign(_reshape_to(dof_velocities, self._dof_velocities.shape))

        if dof_accelerations is None:
            self._dof_accelerations.zero_()
        else:
            self._dof_accelerations.assign(_reshape_to(dof_accelerations, self._dof_accelerations.shape))

        # --- attributes
        if body_com_pos is None:
            self._attributes["body_com"].zero_()
        else:
            self._attributes["body_com"].assign(_reshape_to(body_com_pos, self._attributes["body_com"].shape))

        if body_mass is None:
            self._attributes["body_mass"].zero_()
        else:
            self._attributes["body_mass"].assign(_reshape_to(body_mass, self._attributes["body_mass"].shape))

        if body_inertia is None:
            self._attributes["body_inertia"].zero_()
        else:
            self._attributes["body_inertia"].assign(_reshape_to(body_inertia, self._attributes["body_inertia"].shape))

        if joint_limit_lower is not None:
            self._attributes["joint_limit_lower"].assign(
                _reshape_to(joint_limit_lower, self._attributes["joint_limit_lower"].shape)
            )

        if joint_limit_upper is not None:
            self._attributes["joint_limit_upper"].assign(
                _reshape_to(joint_limit_upper, self._attributes["joint_limit_upper"].shape)
            )
