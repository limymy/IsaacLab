...
def split_spatial_vectory_array_to_linear_velocity_array(
    ...
    Velocities are given in the following format: (vx, vy, vz, wx, wy, wz).
    ...
    """
    index = wp.tid()
    linear_velocity[index] = wp.spatial_top(velocity[index])
...
def split_spatial_vectory_array_to_angular_velocity_array(
    ...
    Args:
        velocity: The spatial velocity. Shape is (num_instances, 6).
        angular_velocity: The angular velocity. Shape is (num_instances, 3). (modified)
    """
    index = wp.tid()
